...
loans = pd.read_csv('https://query.data.world/s/6Vgh0LcibNhMVb-o_0eH97tLqaj6nI',skiprows=1)
...
loan_dict = pd.read_csv('https://query.data.world/s/hdF7Ur3Bfw4-UUI94sEae06XAVvpJG')
...
half_thresh = len(loans) / 2
loans.dropna(thresh=half_thresh, axis=1, inplace=True) 
...
unique_count = loans.apply(pd.Series.nunique)
...
loans = loans.loc[:,unique_count != 1]
...
plt.title('target variable - loan_status (before mapping) ')
